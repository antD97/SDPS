...
/* ----------------------------------------- UI Content ----------------------------------------- */
    ...
    val minimizeSidebarButton = JButton().apply {
        ...
        addActionListener {
            ...
            mainFrame.updateMinSize(false)
        }
    }

    val onTopCheckBox = JCheckBox("Window Always On Top").apply {
        toolTipText = "Force this window to display on top of Smite and other windows (T)"
        isSelected = initConfig.onTop

        addActionListener {
            mainFrame.isAlwaysOnTop = (it.source as JCheckBox).isSelected
        }
    }

    val trackDamageCheckBox = JCheckBox("Track Damage").apply {
    ...
    val trackHealReceivedCheckBox = JCheckBox("Track Heal Received").apply {
    ...
    val trackHealAppliedCheckBox = JCheckBox("Track Heal Applied").apply {
    ...
    val autoResetCheckBox = JCheckBox("Auto-Reset").apply {
        toolTipText = "Automatically reset after 5 seconds of inactivity (Ctrl+R)"
        isSelected = initConfig.autoReset

        addActionListener {
            if ((it.source as JCheckBox).isSelected) autoCombatResetWorker.enable()
            else autoCombatResetWorker.disable()
        }
    }

    val nameField = JTextField(10).apply {
        text = initConfig.ign ?: "Searching..."
        isEditable = false
    }

    private val nameFieldResetButton = JButton("↺").apply {
    ...
    val combatLogField = JTextField(10).apply {
        text = "No file"
        isEditable = false
    }

    private val combatLogResetButton = JButton("↺").apply {
    ...
    private val rowSizeSpinner = JSpinner(SpinnerNumberModel(initConfig.rowSize, 1, 100, 1)).apply {
    ...
    val clearTableButton = JButton("Clear Table").apply {
    ...
    private val obsSettingsButton = JButton("OBS Text Source").apply {
        toolTipText = "Opens the OBS text source settings window"

        addActionListener {
            obsDialog.setLocationRelativeTo(mainFrame)
            obsDialog.isVisible = true
        }
    }
